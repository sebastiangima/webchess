...
var usuariosOnline = {}, unidos = {}, rooms={}, chesses={jugadas:[]}, inRooms={};
   ...
   //     self.port      = 8443;
    ...
    self.createRoutes = function() {
        self.routes = { };
				...
				self.routes['/jugadores']= function(req,res){
					var result=''
					for (var i in unidos) {
						if (result) result+=',';
						result += unidos[i]+','+i
...
io.sockets.on('connection', function(socket) 
{
	...
	socket.on("exitFromRoom", function(u,r)	{

			var uu=rooms[r].users;
			var ii = uu.indexOf(u);
			if(ii!=-1) {
				rooms[r].users.splice(ii,1);
				if(!rooms[r].users.length) {

					delete chesses[rooms[r].chessid];
					delete rooms[r];
					socket.broadcast.emit('closeRoom',r);
				}
			}
	})
	...
	socket.on("newRoom", function(owner,roomid)	{
		...
		if(!inRooms[owner]) inRooms[owner]=[];
		inRooms[owner].push(roomid)
	...
	socket.on("setChessId", function(roomid,chessid) {
		...
		socket.broadcast.emit("onSetChessId", roomid, chessid);
			chesses[chessid]={jugadas:[]};
			console.log(rooms[roomid],chessid,chesses)
	});
	...
	socket.on("changeOptions", function(roomid,option,value) {
		...
		console.log(rooms[roomid],option,value)
	});
	
	socket.on("setColor", function(user,roomid,color,callback) {
		if (rooms[roomid]) {

			var u=rooms[roomid][color];
			if (color=='BLANCAS' || color =='NEGRAS') {
				...
				callback(2,chesses[rooms[roomid].chessid].jugadas)
			}
			if (rooms[roomid].users.indexOf(user)==-1) {
				rooms[roomid].users.push(user)
				if(!inRooms[user]) inRooms[user]=[]
				inRooms[user].push(roomid)
			}
		...
		//socket.broadcast.emit('roomSetColor',user,roomid,color);
		//io.sockets.emit('inRoomSetColor',user,roomid,color);
			console.log(rooms[roomid],user,color)
	});
	...
	socket.on("movimiento", function(from, to, jaque, jugada, coronado)	{
      ...
      socket.broadcast.emit('moving',jugada.chessid,from,to,jaque, coronado,pasarTurno)
		chesses[jugada.chessid].jugadas.push(jugada)
		if (jugada) {
	...
	socket.on("disconnect", function()	{
		...
		if (unidos[socket.username]) {
			io.sockets.emit("noMorePlayer", socket.username);
			delete unidos[socket.username];
		}

		for (var i=0; i<inRooms[socket.username].length; ++i) {
			var r=inRooms[socket.username][i];
			if(!rooms[r]) continue;
			var uu=rooms[r].users;
			var ii = uu.indexOf(socket.username);
			if(ii!=-1) {
				rooms[r].users.splice(ii,1);
				if(!rooms[r].users.length) {
	...
	socket.on("enterRoom", function(roomid,chessid,userid,callback)	{
