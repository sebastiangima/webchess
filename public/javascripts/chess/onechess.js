...
IOneChess.prototype.acomodar = function acomodar(state){    
  ...
  var r=rooms.rooms[this.roomid];
  if (state=='viewer'){
  	this.localRedo=0;
  	this.viewer=true;
  }
  var piezas = window[IOneChess.prototype.proto].prototype.acomodar.call(this);
	this.blanco = new Player( {piezas:piezas['blancas'], handler: this, idcasilla: this.idcasilla, parentNode: this.tablero.element, chessid: this.chessid, name: r.BLANCAS, orientacion:1, color:'blancas'});
  this.negro = new Player( {piezas:piezas['negras'], handler: this, idcasilla: this.idcasilla, parentNode: this.tablero.element, chessid: this.chessid, name: r.NEGRAS, orientacion:-1, color:'negras'});
	return piezas;
}
