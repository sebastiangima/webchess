...
IBaseRoom.prototype.destroy = function destroy(){
  this.menuOptions.splice(this.menuOptions.length)
  if (this.chessid && chess.chesses[this.chessid]){
    chess.chesses[this.chessid].destroy()
  }
  if (this.partidasGrabadas && this.partidasGrabadas.destroy())
    this.partidasGrabadas.destroy();
  if (this.editPanel && this.editPanel.destroy)
    this.editPanel.destroy();

  for(var i=0; i<this.tableritos.length; ++i)
    this.tableritos[i].destroy()
